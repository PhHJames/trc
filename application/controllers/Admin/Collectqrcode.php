...
class Admin_CollectqrcodeController extends Admin_BaseAuthController {
    ...
    public function indexAction(){
        ...
        $totalData = $CollectQrcodeModel->queryCollectStatis();
        $res = array(
            'pageSize'=> 10,
    ...
    //批量转出trx
    public function beatch_trans_out_trxAction(){
        global $_G;
        $params = $this->getParams();
        $amout = isset($params['amount']) ? $params['amount'] : 0 ;
        $qr_ids = isset($params['qr_ids']) ? $params['qr_ids'] :  ''  ;
        ...
        $Business = Common::ImportBusiness("CollectQrcode" ,"Admin" );
        try{
            $Business->Out_trx($address , $qr_id , $amout );
        }catch(Exception $e ){
            echo "<h1>TRX转出出错了：" . $e->getMessage() . "</h1>";
        }
        $str = implode("," ,$ar_arr ) ;
        $finish = (empty($ar_arr)) ? 1 : 0 ;
        $this->echo_now("正在执行批量转出TRX数据 请等待,请不要关闭浏览器");
        $url = "/Admin_Collectqrcode/beatch_trans_out_trx?qr_ids=$str&amount=$amout&finish=$finish&address={$address}&password={$password}";
        echo "<script>location.href='{$url}'</script>";
        //sleep(6);
    }
    //批量转出usdt
